...
{
	ssize_t ret = ::recv(this->fd(), buffer, length, 0);
	validate_ret((int)ret, "socket recv");
...
{
	ssize_t ret = ::send(this->fd(), buffer, length, 0);
	validate_ret((int)ret, "socket send");
